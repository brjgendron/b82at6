(() => {
	...
	let cookieJson = JSON.parse(decodeURIComponent(document.cookie).split("user=j:", -1)[1]);
	// console.log(cookieJson.x);
	...
	socket.on("new connection", (userData_, connectedUsers_) => {
		fillList(usersList, connectedUsers_);
		messages.appendChild(
			genListObj(
				userData_,
				{ html: "span", class: "", contents: "User " },
				{ html: "span", class: userData_.id, contents: `${userData_.username}-${userData_.id}` },
				{ html: "span", class: "", contents: " has connected"}
			)
		);
	});
	...
	socket.on("message", (message_) => {
		messages.appendChild(
			genListObj(
				{ id: message_.senderID, username: message_.senderUsername, color: message_.senderColorHSL },
				{ html: "span", class: "message-timestamp", contents: `[${message_.messageTimestamp}] ` },
				{ html: "span", class: message_.senderID, contents: message_.senderUsernameWithID },
				{ html: "span", class: "message-text", contents: `: ${message_.contents}` }
			)
		);

		document.querySelector("#messages-container").SimpleBar.getScrollElement().scrollTop =
		document.querySelector("#messages-container").SimpleBar.getScrollElement().scrollHeight;
	...
	socket.on("show previous messages", (data_) => {
		messages.appendChild(
			genListObj(
				{ id: data_.senderID, username: data_.senderUsername, color: data_.senderColorHSL },
				{ html: "span", class: "message-timestamp", contents: `[${data_.messageTimestamp}] ` },
				{ html: "span", class: data_.senderID, contents: data_.senderUsernameWithID },
				{ html: "span", class: "message-text", contents: `: ${data_.contents}`}
			)
		);
